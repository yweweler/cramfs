...
static int opt_ignore_errors = 0;   /* ignore errors (-i) */
...
static void die(int status, int syserr, const char *fmt, ...)
{
    ...
    if (opt_ignore_errors == 1
          && ( status == FSCK_NONDESTRUCT
            || status == FSCK_REBOOT
            || status == FSCK_UNCORRECTED
            || status == FSCK_ERROR )) {
        return;
    }
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hx:vi")) != EOF) {
		switch (c) {
		case 'h':
			usage(FSCK_OK);
		case 'x':
        ...
        case 'i':
            if (!opt_extract) {
                die(FSCK_USAGE, 0, "-i can only be used together with -x flag");
            }
#ifdef INCLUDE_FS_TESTS
            opt_ignore_errors = 1;
            break;
